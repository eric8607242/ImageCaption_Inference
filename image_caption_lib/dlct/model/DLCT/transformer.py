...
class Transformer(CaptioningModel):
    def __init__(self, bos_idx, encoder, decoder, box_embed_flag, grid_embed_flag):
        super(Transformer, self).__init__()
        self.bos_idx = bos_idx
        self.encoder = encoder
        self.decoder = decoder

        self.box_embed_flag = box_embed_flag
        self.grid_embed_flag = grid_embed_flag
    ...
    def get_pos_embedding(self, boxes, grids,split=False):
        ...
        if not self.box_embed_flag:
            # print('reach here')
            region_embed = torch.zeros_like(region_embed)
        if not self.grid_embed_flag:
            # print('reach here')
